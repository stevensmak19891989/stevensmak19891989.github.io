...
function generateQRCode() {
    ...
    try {
        if (typeof QRCode !== 'undefined') {
            ...
            qrContainer.innerHTML = '<p style="text-align: center; padding: 20px;">QR Code library not loaded. Please share the link above.</p>';
            console.error('QRCode library not loaded');
        }
    } catch (error) {
        qrContainer.innerHTML = '<p style="text-align: center; padding: 20px;">Error generating QR code. Please share the link above.</p>';
...
function copyPlayerLink() {
    playerLink.select();
    playerLink.setSelectionRange(0, 99999); // For mobile
...
playersButton.addEventListener('click', openPlayersModal);
fullscreenButton.addEventListener('click', toggleFullscreen);
closeSettings.addEventListener('click', closeSettingsModal);
closePlayers.addEventListener('click', closePlayersModal);
saveSettings.addEventListener('click', applySettings);
copyLinkButton.addEventListener('click', copyPlayerLink);
